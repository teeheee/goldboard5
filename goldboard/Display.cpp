...
void Display::printf(const char * format, ...) {
	...
	va_end(argptr);
	BSP_LCD_DisplayStringAtLine((uint16_t)position,(uint8_t*)line);
	position = (position+1)%10;
}

void Display::StringAtLine(uint16_t Line, uint8_t *ptr) {

	BSP_LCD_DisplayStringAtLine(Line, ptr);

}

void Display::StringAt(uint16_t Xpos, uint16_t Ypos, uint8_t *Text,
...
void Display::ClearLine(uint32_t Line) {
...
void Display::PrintNum(uint16_t line, int16_t num) {
...
void Display::PrintNumAt(uint16_t Xpos, uint16_t Ypos, int16_t num,
...
uint8_t Display::P_Touch_GetPositions(void) {
	...
	for (n = 0; n < MultiTouch_Data.cnt; n++) {
		...
		if (position >= TOUCH_MAXY)
			position = (TOUCH_MAXY - 1);
		...
		wert_hi = i2c_wert & FT5336_HI_MASK;
		position = (wert_hi << 8) | wert_lo;
		if (position >= TOUCH_MAXX)
			position = (TOUCH_MAXX - 1);
