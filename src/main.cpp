...
int main(void) {
	...
	I2c.init(3);
	display.init(&I2c);
	display.UB_Touch_Init();
	const char* val = "GoldBoard5";
	const char* val2 = "RoCCI e.V.";
	//display.printf("zahl2 %d ,zahl1 %d",1234,45);
	while (true)
	{
		display.PrintNum(1, (uint8_t) display.P_Touch_ReadID());
		display.ClearLine(1);
	}
	//display.printf(0, (uint8_t*) "IS DES TOUCH?? ");
	/*UB_I2C1_Init();
	 UB_Touch_Init();
	 #define motpin1 0
	 #define motpin2 1
...
/*
 ...
 display.UB_Touch_Read();

 MultiTouch_Data_t MultiTouch_Data = return display.MultiTouch_Data();
 Touch_Data_t Touch_Data= return display.Touch_Data();

 if (display.Touch_Data.status == TOUCH_PRESSED) {
 //gb.DisplaySetTextColor(LCD_COLOR_LIGHTBLUE);
 BSP_LCD_DrawCircle(350,150,100);
 BSP_LCD_FillCircle(350,150,100);
 for(int i=0;i<P_Touch_GetContacts();i++) {
 if((display.MultiTouch_Data.p[i].xp > 250) && (display.MultiTouch_Data.p[i].xp < 450) && (display.MultiTouch_Data.p[i].yp > 50) && (display.MultiTouch_Data.p[i].yp < 250)) {
 ...
 display.printf(1, (uint8_t*) "Rechts ");
 }
 else {
 display.printf(1, (uint8_t*) "Links ");
 }
 }
 else {
 display.printf(1, (uint8_t*) "Zu viele Finger ");
 }
 }
 else {
 //gb.DisplaySetTextColor(LCD_COLOR_LIGHTBLUE);
 ...
 BSP_LCD_FillCircle(350,150,100);
 //gb.DisplaySetTextColor(LCD_COLOR_BROWN);
 display.printf(1,(uint8_t*) "nicht gedrueckt ");
 }
 uint8_t finger = display.P_Touch_GetContacts();

 if (finger == 0) {
 display.printf(0, (uint8_t*) "IS DES TOUCH?? ");
 }
 else if (finger == 1) {
 display.printf(0, (uint8_t*) "1 Finger wurde aufgelegt ");
 }
 else if (finger == 2) {
 display.printf(0, (uint8_t*) "2 Finger wurden aufgelegt ");
 }
 else if (finger == 3) {
 display.printf(0, (uint8_t*) "3 Finger wurden aufgelegt ");
 }
 else if (finger == 4) {
 display.printf(0, (uint8_t*) "4 Finger wurden aufgelegt ");
 }
 else {
 display.printf(0, (uint8_t*) "5 Finger wurden aufgelegt ");
 }
